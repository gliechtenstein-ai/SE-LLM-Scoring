...
# Enrich output
def enrich_with_book_coaching(text, book_chunks, model="gpt-4"):
    from openai import OpenAI
    client = OpenAI()

    # Pre-build the chunk list separately to avoid nested f-string issues
    chunk_block = "".join([f"[{i+1}] {chunk.strip()}\n" for i, chunk in enumerate(book_chunks)])

    # Build enrichment prompt safely
    prompt = (
...
def score_transcript(transcript, config, collections, participants):
    ...
    print("debug 1:", explanations)

    # Enrich each metric explanation


    
    for m_name, data in explanations.items():
        explanation = data.get("explanation", "")

        metric_chunks = collections[framework_name].query(
            query_texts=[explanation],
            n_results=config["settings"].get("max_book_chunks", 3),
            where={"framework_key": m_name}
        )["documents"][0]

        enriched = enrich_with_book_coaching(explanation, metric_chunks, model=model)
        data["explanation"] = enriched 

    print("debug 2:", explanations)
    ...
    summary_text = " ".join([data.get("explanation", "") for data in explanations.values()])
    summary_chunks = collections[framework_name].query(
        query_texts=[summary_text],
        n_results=config["settings"].get("max_book_chunks", 3)
    )["documents"][0]

    
    summary_response = enrich_with_book_coaching(summary_response, summary_chunks)
